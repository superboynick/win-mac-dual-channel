#!/usr/bin/env python3
"""Run the fixed AJM-005 STEP semantic reconstruction diagnostic through MCP."""
...
RUNNER_GIT_PATH = (
    ...
    "run_t1_semantic_reconstruction_suite.py"
)
...
WB_PROFILE = "ajm005-workbench-semantic-reconstruction-t1-v1"
POLL_SECONDS = 1.0
...
PROFILE_RULES = {
    ...
    WB_PROFILE: {
        ...
        "required_status": "PASS_STEP_SEMANTIC_RECONSTRUCTION_DIAGNOSTIC",
        "assertion_field": "diagnostic_assertions",
        "report": "workbench_semantic_reconstruction_t1.json",
        "artifacts": {
            "workbench_semantic_reconstruction_t1.wbpj": "project",
            "workbench_semantic_reconstruction_inspection.json": "model_inspection",
        },
        "assertions": {
            "predecessor_identity",
            "semantic_sidecar_identity",
            "body_geometry_available",
...
def require_runner_git_blob(head: str) -> str:
    ...
    if completed.returncode != 0:
        raise RuntimeError("BLOCKED_T1_SEMANTIC_RUNNER_GIT_BLOB_MISSING")
    ...
    if installed_digest != git_digest:
        raise RuntimeError("BLOCKED_T1_SEMANTIC_RUNNER_COPY_MISMATCH")
...
async def run_profile(
    ...
    assertions = (
        report.get(rule.get("assertion_field", "assertions"))
...
async def run_suite() -> int:
    ...
    stderr_path = (
        OUTPUT_ROOT / f"AJM005_T1_SEMANTIC_RECON_SUITE_{stamp}_MCP_STDERR.log"
    )
    result: dict[str, Any] = {
        "schema_version": 1,
        "task": "AJM-WIN-ANSYS-STUDENT-CAPABILITY-SMOKE-005",
        "suite": "step_semantic_reconstruction_t1_diagnostic",
        "case_id": case_id,
        "started_at": utc_now(),
        "ended_at": None,
        "suite_status": "FAIL_STEP_SEMANTIC_RECONSTRUCTION_DIAGNOSTIC",
        "p1_cad_toolchain_readiness": "BLOCKED",
        "p1_cad_blocker": (
            "NATIVE_PARAMETERIZATION_AND_NATIVE_TRANSFER_NOT_PROVEN"
        ),
        "pass_005_capability": "DIAGNOSTIC_ONLY_NOT_CAD_TRANSFER",
        "p1_p6_gates": "NOT_RUN",
        "visibility": "NOT_USER_OBSERVED",
        "license_arguments_added": False,
        "semantic_reconstruction_diagnostic": "NOT_RUN",
    ...
    try:
        if norm(Path(sys.executable)) != norm(EXPECTED_PYTHON):
            raise RuntimeError("BLOCKED_WRONG_T1_SEMANTIC_RUNNER_INTERPRETER")
                ...
                async with ClientSession(
                    *streams,
                    ...
                    client_info=types.Implementation(
                        name="airjet-ajm005-t1-semantic-reconstruction-harness",
                        version="1.0.0",
                    ),
                ) as session:
                    ...
                    if sc_run["capability_pass"]:
                        ...
                        wb_diagnostic_assertions = (
                            wb_report.get("diagnostic_assertions", {})
                            if isinstance(wb_report, dict)
                            else {}
                        )
                        if (
                            isinstance(wb_report, dict)
                            and wb_report.get("status")
                            == "PASS_STEP_SEMANTIC_RECONSTRUCTION_DIAGNOSTIC"
                            and all(
                                wb_diagnostic_assertions.get(name) is True
                                for name in (
                                    "predecessor_identity",
                                    "semantic_sidecar_identity",
                                    "body_geometry_available",
                            ...
                            and wb_report.get("diagnostic_result", {}).get(
                                "native_named_selection_transfer_claim"
                            )
                            is False
                        ):
                            result["semantic_reconstruction_diagnostic"] = "PASS"
                        else:
                            result["semantic_reconstruction_diagnostic"] = "FAIL"
                    else:
                        result["runs"].append(
                            {
                    ...
                    if (
                        ...
                        and result["semantic_reconstruction_diagnostic"] == "PASS"
                    ):
                        result["suite_status"] = (
                            "PASS_STEP_SEMANTIC_RECONSTRUCTION_DIAGNOSTIC"
                        )
