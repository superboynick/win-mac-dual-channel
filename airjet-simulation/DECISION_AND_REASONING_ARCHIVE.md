...
## 21. 为什么下一步采用 STEP 归档 + Parasolid x_t solver 候选，而不是直接重建九变体

当前失败发生在表示传递而非 native producer：SpaceClaim native 中两侧孔口面存在，STEP 到
Mechanical 后只剩单侧。最小区分实验必须保持完整 12-cell/972-hole V02、参数和观察器目标不变，
只改变 solver handoff 表示。SpaceClaim 使用 Parasolid 内核，因此 x_t 是值得检验的候选；但它不是
产品事实，也不能替代 STEP 的通用归档职责。

为避免“换格式后只看 body count 就通过”，pilot 拆成同一 MCP 进程的三段冻结链。converter 从
predecessor manifest 约束的 native 建立 job-local writable staging，只声明 x_t 回读的两体
closed/manifold、face count、bbox/volume envelope 在容差内；明确
`representation_conversion=true`、`source_native_mutated=false`，接口拓扑留给 observer。observer
再按 body name/z 绑定角色，逐角色核对 face count/bbox/volume，用 972 点合同和逐对
centroid/plane/bbox/area/normal 检查 coincident faces，并把 `AdjacentBodies` 纳入 shared membership。

静态终审还要求 predecessor 完整目录成员、manifest、size/SHA 前后相同，staging 不出现额外文件，
artifact manifest 的 job/phase 与 terminal state 一致。`PASS_OBSERVATION` 与
`PASS_CANDIDATE_ROUTE_TO_MESH` 分离；后者仍不证明 mesh/shared nodes/P1。若 x_t 失败，下一步是
显式 upstream interface reconstruction，而不是放宽识别阈值；若 x_t 成功，仍要单独做 mesh 诊断后
才讨论正式 006 production profiles。

截至当前只完成 Mac 静态包：16 项 runner guard、14-profile MCP policy、144-file audit PASS；
Windows/ANSYS terminal 证据不存在，P1--P6 均 `NOT_RUN`。
