# AJM-006 formal full-product two-zone SpaceClaim producer candidate.
# Registration remains prohibited until the semantic bundle and observer are complete.
...
report_path = os.path.join(job_dir, "full_product_producer.json")
authoring_path = os.path.join(job_dir, "full_product_authoring.scdocx")
...
inventory_path = os.path.join(job_dir, "full_product_face_inventory.json")
...
DEPENDENCY_NAMES = (
    ...
    "campaign.json",
    "variant_01_m_3x4_7_0_r25_bottom_heavy.json",
    "variant_02_m_3x4_7_0_r50_balanced.json",
    "variant_03_m_3x4_7_0_r75_top_heavy.json",
    "variant_04_m_s_3x5_6_0_r50_balanced.json",
    "variant_05_l_2x4_8_0_r50_balanced.json",
    "variant_06_s_3x5_5_5_r50_balanced.json",
    "variant_07_m_3x4_7_0_r50_vent_upper.json",
    "variant_08_m_3x4_7_0_r50_orifice_edge_gap.json",
    "variant_09_m_3x4_7_0_r50_exhaust_half_taper.json",
)
...
result_data = {
    "schema_version": 1,
    "task": "AJM006_FORMAL_FULL_PRODUCT_PRODUCER_CANDIDATE",
    "probe": "full_product_producer",
    "status": "FAIL_PRELIMINARY_GEOMETRY",
    "engineering_capability": "FAIL_PRELIMINARY_GEOMETRY",
    "claim_scope": "FORMAL_PRODUCER_IMPLEMENTATION_INCOMPLETE_NOT_REGISTERABLE",
    "formal_006_completion": False,
    "p1_stage_gate": "NOT_RUN",
    "p1_p6_gates": "NOT_RUN",
    "full_variant_campaign": "ONE_TRUSTED_VARIANT_PER_JOB_PENDING_9_JOB_CAMPAIGN",
    ...
    "top_plenum_planform": "C_CLASS_REAR_SUPPORT_EXTENSION_WITH_FOUR_VENT_RISERS",
...
def derive_supported_plenum_rear(vent_records, footprint_y_min):
    if len(vent_records) != 4:
        raise Exception("AJM006_FORMAL_VENT_SUPPORT_COUNT_NOT_4")
    by_id = {}
    for vent in sorted(vent_records, key=lambda item: item["vent_id"]):
        vent_id = str(vent["vent_id"])
        if vent_id in by_id:
            raise Exception("AJM006_FORMAL_VENT_SUPPORT_DUPLICATE_ID")
        box = compute_vent_box(vent)
        if box[0] >= box[2] or box[1] >= box[3]:
            raise Exception("AJM006_FORMAL_VENT_SUPPORT_BOX_INVALID")
        by_id[vent_id] = box
    if set(by_id) != set(["V01", "V02", "V03", "V04"]):
        raise Exception("AJM006_FORMAL_VENT_SUPPORT_IDS_INVALID")
    supported_y_min = min(
        [float(footprint_y_min)] + [box[1] for box in by_id.values()]
    )
    rear_ids = sorted(
    ...
    support = {
        ...
        "rear_support_extension_mm": float(footprint_y_min) - supported_y_min,
        "rear_inlet_ids": rear_ids,
        "vent_boxes_mm": by_id,
    }
    if (
        rear_ids != ["V01", "V02"]
        or supported_y_min >= float(footprint_y_min)
        or support["rear_support_extension_mm"] <= 0.0
    ):
        raise Exception("AJM006_FORMAL_VENT_SUPPORT_CONTRACT_MISMATCH")
    return support
...
try:
    ...
    if len(records) != 1:
        raise Exception("AJM006_FORMAL_CAMPAIGN_RECORD")
    record = records[0]
    ...
    pitch = membrane + wall
    if cell_count != nx * ny or cell_count not in (8, 12, 15) or len(vents) != 4:
        raise Exception("AJM006_FORMAL_FULL_PRODUCT_CARDINALITY")
    ...
    expected_orifice_count = len(orifice_entities)
    if expected_orifice_count <= 0 or expected_orifice_count % cell_count != 0:
        raise Exception("AJM006_FORMAL_ORIFICE_COUNT")
    ...
    if (
        ...
        and close_enough(footprint_y_min, -14.5, 1.0e-9)
        and (
            not close_enough(
        ...
        raise Exception("AJM006_FORMAL_ACCEPTED_REAR_SUPPORT_REGRESSION")
    ...
    for vent in sorted(vents, key=lambda item: item["vent_id"]):
        ...
        risers.append(create_block(
            ...
            "AJM006_FORMAL_RISER_%s" % vent["vent_id"],
        ))
    ...
    group_required = {
        ...
        "INLET": 4,
        "OUTLET": 1,
        "MEMBRANE_TOP": cell_count,
        "MEMBRANE_BOTTOM": cell_count,
        "ORIFICE_EXIT_UPSTREAM": expected_orifice_count,
        "ORIFICE_ENTRY_DOWNSTREAM": expected_orifice_count,
        "HEAT_WALL": 1,
    }
    ...
    result_data["assertions"]["full_product_scope"] = (
        cell_count == nx * ny
        and cell_count in (8, 12, 15)
        and len(orifice_entities) == expected_orifice_count
        and len(cell_origins) == cell_count
    ...
    result_data["assertions"]["complete_flow_path"] = (
        ...
        and group_observed["ORIFICE_EXIT_UPSTREAM"] == expected_orifice_count
        and group_observed["ORIFICE_ENTRY_DOWNSTREAM"] == expected_orifice_count
    )
...
if result_data["status"] != "PASS_PARTIAL_CAD_CAPABILITY":
    raise Exception("AJM006_FORMAL_PRODUCER_CANDIDATE_FAILED")
