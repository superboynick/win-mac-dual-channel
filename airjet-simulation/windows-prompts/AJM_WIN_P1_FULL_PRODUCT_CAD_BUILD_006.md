...
外包络只能锁定 `27.5 x 41.5 x 2.8 mm`。所有 `CAND/REF/U` 名称和证据标签保留。`C017_SUPPORT_ALLOWANCE_REF`、`C019_TOP_REF`、`C019_BOTTOM_REF` 只能是 construction/reference geometry：不得参与 Boolean、不得赋材料/质量、不得传给结构或 CHT、不得作为物理产品层导出。另建的 `FLUID_DOMAIN_CLOSURE_DATUM_C` 只能按 `RESIDUAL_NUMERICAL_CLOSURE_R0` 提供流体边界 datum；它不是残差实体。`CENTRAL_ANCHOR_CAND_TEMPLATE` 是 P 存在/拓扑 prior 下的 C 类方形 construction datum，`CELL_PARTITION_CAND_TEMPLATE` 是零厚度 ownership/naming datum；二者均无材料、质量、Boolean、solver/export，且不得进入 fluid union。唯一局部例外是单独声明的 `VENT_RISER_FLUID_CAND`：它只能在所选四个 vent slot footprint 内从 top-plenum 顶面延伸到产品顶面并 union 进上游流体；这只是 C/U 类连通候选，不等于 C019 整层为空气，也不是被图像观察到的生产风道。`FLEX_KEEP_OUT_U` 仅为参考禁入区。

流体域必须直接构造合同声明的四个 vent riser、top plenum、perimeter gaps、bottom chambers、orifice throats、impingement channel、manifold 和 spout。vent riser、top plenum、perimeter gaps、bottom chambers、orifice throats union 为一个 upstream body；impingement channel、manifold、spout union 为一个 downstream body；两体在每个 orifice exit 保持共享接口且不得互相 union。再做原生重开、STEP 重导入和连通复核。禁止使用“整个外包络减去所有候选固体”作为流体域，因为那会把 `C017/C019` 未识别区错误地当成空气。
...
selected_vent_riser_rule_id=VENT_RISER_CANDIDATE_R0
...
- `PARTIAL_CAD_OUTPUT`：已经开始建模，但 4 配置、9 个有独立 ID/Gate 的 variants、单因素 diff、
  10 条内部几何规则（含仅限四个 vent 投影的局部候选 riser）、原生重开、完整连通、零孤立体、零干涉/碎片/盲孔、required semantic keys、
