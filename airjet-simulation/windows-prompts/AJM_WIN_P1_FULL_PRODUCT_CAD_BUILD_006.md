...
006 绝不输出 `P1_STAGE_GATE=PASS`。即使全部自动检查成功，也只能写 `PENDING_MAC_REVIEW`，保留产物并停止，等待 Mac 端独立审计和用户查看。
...
- `COMPLETE_WITH_TRANSFER_LIMITATION_AWAITING_REVIEW`：上一条全部完成，唯一剩余失败是 005 已知或 006 复现的 STEP 导出/重导入限制；`P1_STAGE_GATE=PENDING_MAC_REVIEW`。
- `COMPLETE_AWAITING_REVIEW`：4 个配置、9 个变体及全部硬检查、原生/STEP/Workbench 传递和 manifest 均完成；`P1_STAGE_GATE=PENDING_MAC_REVIEW`。
