...
## 30. 第二十二次实验设计：file-only reachability（历史 pre-run 设计；实跑见 30.7）
...
### 30.7 实跑填写：call 返回，entry/build 未观测

| field | actual value |
|---|---|
| signed run commit | `1a9696c3930a42cd8a30aafe7093b8acafd6dd59` |
| suite / case | `AJM005_T1_CONNECTED_SC_SUITE_20260715T021529059815Z_aa1180f6` / `a5c-eedabacc1fc6` |
| producer job | `a5c-eedabacc1fc6-f70b77c399ca`; `PASS_PARTIAL_CAD_CAPABILITY`; 8/8 assertions |
| consumer job | `a5c-eedabacc1fc6-027f5de8b724`; `FAILED_PROCESS`; exit 2 |
| RunScript outcome | `RETURNED` |
| entry first observed | not observed at any of four checkpoints |
| freeze / capture build state | `ABSENT` / `ABSENT`; no probe error |
| exact classification | `RUNSCRIPT_RETURNED_ENTRY_ABSENT` |
| root error | `FAIL_RUNSCRIPT_RETURNED_ENTRY_AND_BUILD_ABSENT` |
| engineering downstream | share/save-data/Refresh/Mechanical/mesh/project `NOT_REACHED` |
| visibility / P1 Gate | `NOT_USER_OBSERVED` / `NOT_RUN` |

`RETURNED` 是 outer Workbench API call 的事实；entry absent 是本轮 fixed marker 在有限检查点的事实。
两者组合不能推出 child build 已执行后失败。特别是 build contract 虽在 outer journal 标为 `CALLED`，
但 build JSON 从未被观察到，因此 connected transfer 既未通过，也没有被本轮执行到足以判定失败。

本轮 raw evidence 的 producer 20 项、consumer 19 项 manifest 已逐项重算一致。suite JSON SHA-256 为
`5069fc1a6681cf54f38ac4c8a0793f09cabc3ed9935cd422c57424539e08cadb`；Git 外 ZIP SHA-256 为
`62b058ef4125704ef4d74624d23b5cc0093315ab29bc613cd0e55cf5d92b7a96`。suite 结束瞬间没有外层
process observation，归档时的 0-process 延迟检查不能冒充即时 cleanup 证据。

本实验系列在当前 host 到此冻结为 `DEFERRED_CURRENT_HOST_ROUTE`。下一 workbook 主线是
`SPACECLAIM_SIGNED_SCRIPT_PARAMETRIC → HASH_BOUND_STEP_SEMANTIC_SIDECAR`，并用逐实体 key、owner、
cell/local coordinates、cardinality、adjacency 与全链 hashes fail closed。native 三项主张保持
`NOT_PROVEN`，P1 Gate 保持 `NOT_RUN`。
