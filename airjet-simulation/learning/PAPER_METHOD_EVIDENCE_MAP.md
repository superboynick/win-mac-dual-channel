...
| Methods / CAD readiness | 脚本参数驱动流体几何是否可传递 | 005 T1 partial | 可删除流道 | C | 005 §3 尺寸 | 两次脚本重建、negative-volume union、原生/STEP 重开、Workbench/Mechanical transfer | `ajm005-spaceclaim-cad-t1-v1` + `ajm005-workbench-transfer-t1-v1` | 解析体积/bbox、单连通体、Named Selection cardinality、粗网格、报告—manifest 交叉哈希 | 首轮 `...T185233...`：参数重建 PASS，SC group array 类型 FAIL；第二轮 `...T190312...`：类型修复有效，但指纹发现入口缺失，STEP 根层 query 为 0；两轮 WB 均 BLOCKED，第三版修复待签名重跑 | 当前可写“脚本参数重建已实测；解析指纹阻止了 Boolean Success 但缺入口的错误几何下传，并暴露 v261 类型问题以及 STEP 根层查询/导入路线未决问题” | 原生 driving parameter 与实际 Volume Extract 均 NOT_RUN；三段 union、完整 Named Selections、STEP 可移植性及 CAD transfer 未通过；不等于 P1 CAD readiness 或 P1 整机 CAD | PARTIAL_FAIL_PRESERVED |
