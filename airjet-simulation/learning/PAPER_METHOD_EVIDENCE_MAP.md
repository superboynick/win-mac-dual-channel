...
| Methods / diagnostic instrumentation | file-only RunScript 诊断能否无歧义地区分 call、entry timing 与 build state | 005 T1 run #22 | 可删除 connected fixture | C | run #20/#21 reach、fixed 34-byte entry sentinel、literal path child | direct RunScript-only；normal/failure checkpoint；freeze/capture 正交状态机 | `ajm005-workbench-connected-spaceclaim-t1-v1`；outer journal + connected suite runner | exact size/SHA；outcome/reach/checkpoint/first-observed/delayed/lost/build-state 交叉约束；AST owner/cardinality/order/alias/reflection mutations；pre-run local static review 记录于 REAL-049 | `NONE`；Windows runtime、run/job、declared report、artifact manifest 均 `NOT_RUN` | 仅内部方法开发记录可写“实现了一套待运行验证的 fail-closed file-only diagnostic instrumentation”；不得作为论文运行证据 | 不能写 RunScript 已返回或抛错、entry 已出现或缺失、`.py` loader 可用或不可用、connected transfer/mesh/project 通过、GUI 可见、TOCTOU 已消除、AST guard 是 runtime sandbox，或 P1 已通过；freeze/capture 非原子 | NOT_RUN |
...
- run #22 只有同时具备签名 commit/profile SHA、run/job ID、declared report、artifact manifest、精确
  classification 与 visibility 后，才能从 implementation row 晋升为运行证据。
