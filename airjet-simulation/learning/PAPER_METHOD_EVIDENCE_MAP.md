...
| Methods / CAD readiness | 脚本参数驱动流体几何是否可传递 | 005 T1 partial | 可删除流道 | C | 005 §3 尺寸 | 两次脚本重建、negative-volume union、原生/STEP 重开、Workbench/Mechanical transfer | `ajm005-spaceclaim-cad-t1-v1` + `ajm005-workbench-transfer-t1-v1` | 解析体积/bbox、单连通体、Named Selection cardinality、粗网格、报告—manifest 交叉哈希 | 前三轮保留 SC API/几何失败；第四轮起 SC 七项连续 PASS；第四至九轮逐项排查 native direct/share/update/editor routes；第十轮 STEP 诊断得到 1 body、1063 nodes、513 elements 和已保存 project，但三组上游名称对象/实体全为 0，canonical native assertions 按设计为 false | 当前可写“签名小模型已实测脚本参数重建、解析三段 union、边界命名、原生保存重开与 STEP 全层几何回读；冻结 predecessor 身份通过；STEP 诊断证明 Workbench→Mechanical body/mesh/project 管线可用，但没有观察到上游三组语义” | 原生 driving parameter 与实际 Volume Extract 均 NOT_RUN；native `.scdocx` attach 和 Named Selection transfer 仍失败；STEP 的 solver-side semantic reconstruction 尚未运行；不等于 P1 CAD readiness 或 P1 整机 CAD | SC_PARTIAL_PASS_STEP_PIPELINE_DIAGNOSTIC_PASS_NATIVE_SEMANTIC_FAIL_PRESERVED |
