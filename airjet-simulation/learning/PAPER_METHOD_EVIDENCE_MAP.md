...
| Methods / CAD readiness | 脚本参数驱动流体几何是否可传递 | 005 T1 partial | 可删除流道 | C | 005 §3 尺寸 | 两次脚本重建、negative-volume union、原生/STEP 重开、Workbench/Mechanical transfer | `ajm005-spaceclaim-cad-t1-v1` + `ajm005-workbench-transfer-t1-v1` | 解析体积/bbox、单连通体、Named Selection cardinality、粗网格、报告—manifest 交叉哈希 | 前三轮保留 SC API/几何失败；第四轮起 SC 七项连续 PASS；第四/五轮两种 direct-attach update 路线均 attach FAIL；第六轮独立 Geometry source 的 `TransferData` 在 component 兼容性检查处直接拒绝 | 当前可写“签名小模型已实测脚本参数重建、解析三段 union、边界命名、原生保存重开与 STEP 全层几何回读；冻结 predecessor 身份已在 Workbench 前置检查通过；direct attach 与普通 Geometry TransferData 路线均未成功” | 原生 driving parameter 与实际 Volume Extract 均 NOT_RUN；Workbench geometry transfer 失败，Named Selection inspection、粗网格和 project save 未到达；不等于 P1 CAD readiness 或 P1 整机 CAD | SC_PARTIAL_CAD_PASS_WB_GEOMETRY_ROUTES_FAIL_PRESERVED |
