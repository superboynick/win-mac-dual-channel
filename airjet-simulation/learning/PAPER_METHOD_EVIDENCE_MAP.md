...
| Methods / CAD readiness | 脚本参数驱动流体几何是否可传递 | 005 T1 partial | 可删除流道 | C | 005 §3 尺寸 | 两次脚本重建、negative-volume union、原生/STEP 重开、Workbench/Mechanical transfer | `ajm005-spaceclaim-cad-t1-v1` + `ajm005-workbench-transfer-t1-v1` | 解析体积/bbox、单连通体、Named Selection cardinality、粗网格、报告—manifest 交叉哈希 | 前三轮保留 SC API/几何失败；第四轮起 SC 七项连续 PASS；第四至九轮逐项排查 native routes；第十轮 STEP 诊断得到 1 body、1063 nodes、513 elements 和 project，但上游名称为 0；第十一次长 job path在 Mechanical 前 attach 失败；第十二次短路径到达分类并拒绝 0/1/12；第十三次证明入口 center 稳定、area 不稳定；第十四次观测 face 44 为 plane/2-edge/z-normal 且全部 topology APIs 无 error | 当前可写“签名小模型已实测脚本参数重建、解析三段 union、边界命名、原生保存重开与 STEP 全层几何回读；STEP 诊断证明 Workbench→Mechanical body/mesh/project 管线可用；独立 reconstruction 已验证输入身份、短路径可达性、失败分区控制，并建立 solver-side topology calibration” | 原生 driving parameter 与实际 Volume Extract 均 NOT_RUN；native `.scdocx` attach 和 Named Selection transfer 仍失败；solver-side reconstruction/mesh 仍 FAIL；topology predicate 尚未实跑；不等于 P1 CAD readiness 或 P1 整机 CAD | SC_PARTIAL_PASS_STEP_PIPELINE_DIAGNOSTIC_PASS_SEMANTIC_TOPOLOGY_CALIBRATION_AVAILABLE_PARTITION_FAIL_PRESERVED |
