...
- V02 native 0.5 mm 无物理 Mechanical 共节点诊断已实跑：producer PASS，但 observer #1 在 hash-equal staging 的 `Model.Refresh()` attach 失败，Mechanical/mesh 未到达，observer #2 未提交。native 路线累计一 PASS/两 attach FAIL，三个 producer native SHA 均不同；该路线不再作为 formal P1 前置依赖。
- V02 split STEP fallback 已在 commit `0491f2a0bf29de03538e4902a2962748402dc42f` 实跑并 fail closed：upstream STEP 回读为 1 个 closed/manifold body、2044 faces，但 bbox 最大分量漂移 0.014975 mm；downstream STEP 包络/体积保持却从 978 faces 愈合为 6 faces，972 个零厚度界面印记消失。WorkBench/mesh/physics 未运行；该表示路线关闭，但不是整机几何失败。
