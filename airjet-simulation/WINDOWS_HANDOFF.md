...
17. `airjet-simulation/windows-prompts/AJM_WIN_V02_PRELIMINARY_006.md`（已完成的 producer 历史入口；当前不重复）
18. `airjet-simulation/windows-prompts/AJM_WIN_V02_TOPOLOGY_OBSERVER_006.md`（已完成的 observer 历史入口；当前不重复）
19. `airjet-simulation/windows-prompts/AJM_WIN_P1_FULL_PRODUCT_CAD_BUILD_006.md`（接口传递修复并由新 observer 关闭后才执行）
20. `airjet-simulation/checklists/P1_CAD_INDEPENDENT_REVIEW_METHOD.md`（006 完成后由独立审查端执行）
...
`AJM-P0-v001` 公开证据冻结已通过；尚未进入 P1 CAD 定版。第三方 PLE 已清理，当前为纯净官方 Ansys Student 2026 R1 基线；005 alternate-route v2 已通过，native 三项仍未证明。用户另行提交了 30 天官方试用申请，但 entitlement 尚未确认激活。Windows Codex 当前应先：
...
4. V02 固定 producer 与修正版 topology observer 均已实跑 PASS；不要无差别重复运行。observer commit `9699df...` 的结果是 upstream body 4288/100 faces、downstream body 7231/978 faces，downstream 972 imprint 完整而 upstream 972-interface 全失。Downloads 证据 ZIP 为 20441827 bytes、SHA256 `10bb50254101b05944c44507077975a92e8b416e7107d12494350c2a4eb377bd`；不把无头结果写成 GUI visible PASS；
5. 当前 STEP→Workbench/Mechanical 两区路线已被 observer 否决，Windows 不得启动正式 006，也不得直接 RunWB2 做临场 workaround。等待新的“接口传递表示修复”签名 commit/profile；新路线必须重新实测两侧 actual IDs、owner/adjacency 与 solver-side 972-interface 可重建性。关闭后才用同一 27.5 x 41.5 x 2.8 mm 母版建立全部 9 个 variant。输入由两个 P1 生成器、10 条 internal R0 和 `geometry/contracts/` 锁定，不能由 Windows 临场猜测；vent riser 只允许存在于四个所选 vent 投影内；006 必须保留真实 005 副本、固定角色 manifest、机器检查和 252 行 evidence；
...
当前 runtime 已允许用户从可见桌面显式启动手动常驻 watcher；禁止直接绕过 manager
调用 watcher/runner。默认轮询周期为 10 秒。Windows 隔离测试固定要求
`WINDOWS_CORE_CASES_PASS=54`、`EXPECTED_PASS_COUNT=54` 与
`OVERALL=PASS_CORE_RUNTIME_ENABLED_MANUAL`。

当前未授权开机启动，不得把 watcher 注册为当前用户的
...
V02 producer 和 STEP topology observer 都已完成；当前没有授权执行的正式 006 production profile。
下一诊断入口已固定为 `windows-prompts/AJM_WIN_V02_PARASOLID_TOPOLOGY_OBSERVER_006.md`：只有
Mac 给出 signed `GIT_READY=<40hex>` 且用户明确通知 Windows 恢复后，才可 `pull --ff-only` 并运行
该文件指定的同进程三阶段 runner。不得直接 RunWB2、重复独立 producer/observer、启动网格或九变体。
若 30 天官方试用开通，再使用 004。旧 001、003、005、V02 producer/STEP observer 都保留作历史记录。
