...
17. `airjet-simulation/windows-prompts/AJM_WIN_V02_PRELIMINARY_006.md`（当前下一轮入口：只跑 producer）
...
4. 安装/验证最新 `airjet-ansys-automation` skill 与 MCP，再执行 `windows-prompts/AJM_WIN_V02_PRELIMINARY_006.md` 的固定 runner；不直接调用 ANSYS，不等待 30 天试用审批，也不把无头结果写成 GUI visible PASS；
5. V02 producer 只生成主候选的 12-cell/972-hole 两区 preliminary CAD，用于实测 Boolean、孔口 shared/coincident topology 与 native/STEP 稳定性；它不能宣告 P1 PASS。关闭 topology schema 后再执行正式 006，用同一 27.5 x 41.5 x 2.8 mm 母版建立全部 9 个 variant。输入由两个 P1 生成器、10 条 internal R0 和 `geometry/contracts/` 锁定，不能由 Windows 临场猜测；vent riser 只允许存在于四个所选 vent 投影内；006 必须保留真实 005 副本、固定角色 manifest、机器检查和 252 行 evidence；
...
当前下一轮入口是 `airjet-simulation/windows-prompts/AJM_WIN_V02_PRELIMINARY_006.md`。只跑 producer；observer 尚未注册，不得直接 RunWB2。若 30 天官方试用开通，再使用 004。SSH/Git 可以可靠同步这些文件；旧 001、003 和已完成的 005 保留作历史记录，不再作为当前入口。
