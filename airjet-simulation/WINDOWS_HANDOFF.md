...
18. `airjet-simulation/windows-prompts/AJM_WIN_P1_FULL_PRODUCT_CAD_BUILD_006.md`（V02 topology 结果关闭并注册正式 profiles 后执行）
...
4. V02 固定 producer 已在签名 commit `64b57303...` 实跑 `PASS_PRELIMINARY_PRODUCER`；不要无差别重复运行。先核验 `logs/evidence/AJM006_V02_PRELIMINARY_20260715T113939945030Z_1082d551ee85/` 与 Downloads ZIP 哈希；不把无头结果写成 GUI visible PASS；
5. 下一步由 Mac 侧先根据 V02 的真实 native/STEP face decomposition 编写、静态审核并签名注册 observer。Windows 在收到新的签名 commit/固定 runner 前不得直接 RunWB2。observer 必须实测 shared/coincident actual IDs 与 solver-side 972-interface 可重建性；关闭 topology schema 后再执行正式 006，用同一 27.5 x 41.5 x 2.8 mm 母版建立全部 9 个 variant。输入由两个 P1 生成器、10 条 internal R0 和 `geometry/contracts/` 锁定，不能由 Windows 临场猜测；vent riser 只允许存在于四个所选 vent 投影内；006 必须保留真实 005 副本、固定角色 manifest、机器检查和 252 行 evidence；
...
V02 producer 已完成；observer 尚未注册，因此 Windows 当前没有可执行的新 ANSYS profile，不得直接 RunWB2 或重复 producer。等待 Mac 推送新的签名 observer task 后再 `pull --ff-only`、复核并执行。若 30 天官方试用开通，再使用 004。SSH/Git 可以可靠同步这些文件；旧 001、003、005 和 V02 producer 都保留作历史记录，不再作为下一轮入口。
