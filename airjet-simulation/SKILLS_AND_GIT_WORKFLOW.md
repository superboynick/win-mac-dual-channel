...
Mac 的手动循环 Git 检查与可见 Codex 唤醒脚本位于 `tools/airjet-git-watcher/`。本地 bare-Git 隔离矩阵的 58 个核心断言已通过：普通更新只同步不唤醒，只有变化且有效的 `target=mac` 信封才能建立 wake pending；重试前会重新核验 Git 并从提交重建任务状态；脏工作树、ahead、分叉、错误远端、watcher 自更新、路径穿越、symlink/tree 任务对象和状态/报告目录越界均 fail closed。当前仍为 `DISABLED_PENDING_HARDENING`，`start/retry` 有代码锁；提交认证策略与用户可见唤醒尚未复核，所以不得启动，也不得安装 LaunchAgent、cron 或 Login Item。
