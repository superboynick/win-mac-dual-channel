...
| `airjet-ansys-automation` | 项目自建 | 固定 profile 的 SpaceClaim/Workbench/PyMechanical/PyFluent 自动化、运行证据和安全边界 | 当前私有仓库 |
...
脚本同步两个项目 skill，检查两个官方 skill；官方 skill 缺失或版本不同时，从锁定的 OpenAI commit 下载；最后核对四个入口文件 SHA256，并确认清单声明的全部必需文件存在。
...
Mac 脚本同步两个项目 skill，并核对当前已安装的两个官方 skill。官方 skill 更新应使用 `skill-installer`，然后有意识地更新锁文件。
...
Claude CLI 可作为第二模型审查资源，双端统一入口见 `tools/claude-cli/README.md`。
默认只把 staged diff 交给无工具、plan 模式、无会话持久化的 reviewer；Codex 保留
任务、测试和 Git 所有权。`dsv4pro` 只是口头简称，CLI 必须使用
`deepseek-v4-pro`；批量低风险扫描可使用 `deepseek-v4-flash`。本机
`~/.claude/settings.json`、token、gateway URL、session 和 cache 不得进入 Git。

Mac 的手动循环 Git 检查与可见 Codex 唤醒脚本位于 `tools/airjet-git-watcher/`。本地 bare-Git 隔离矩阵固定为 80 项：普通更新只同步不唤醒，只有变化且有效的 `target=mac` 信封才能建立 wake pending；重试前会重新核验 Git 并从提交重建任务状态；脏工作树、ahead、分叉、错误远端、watcher 自更新、路径穿越、symlink/tree 任务对象和状态/报告目录越界均 fail closed。runtime 已启用为用户显式启动的手动常驻模式；不得安装 LaunchAgent、cron 或 Login Item，除非用户另行明确授权开机启动。

## 更新规则

1. 修改仓库中的对应项目 skill：`airjet-product-reconstruction` 或 `airjet-ansys-automation`。
