...
Mac 的手动循环 Git 检查与可见 Codex 唤醒脚本位于 `tools/airjet-git-watcher/`。它默认不运行，也不安装 LaunchAgent、cron 或 Login Item；只有用户明确要求启动时，才从本机可见 Terminal 运行该目录 README 中的 `start` 命令。
