...
**真机更新**：commit `ca62c01...` 的 high-resolution consumer 已通过 STEP import、4/1/972 roles、
0.05 mm throat sizing、surface mesh 与 boundary type update；Fluent internal telemetry 达到
1,580,277 cells/12 zones/min OQ 0.23064141 后触发 Student license。因脚本 postconditions 与 `.msh.h5`
write 未到达，本轮为 mesh capability FAIL，不改变 P1 Gate。

## AJM-P1-MESH-002：V03 Student-coarse topology candidate C1

日期：2026-07-15
状态：Windows 实跑完成；Student scale PASS、单 common fluid-zone FAIL

**单一改变**：保留 V03 完整产品候选与 972 个 throat，把 high-resolution 的 throat/surface/volume
尺寸从 `0.05 / 0.025--0.5 / 0.5 mm` 改为 C1 的 `0.075 / 0.05--0.75 / 0.75 mm`。这是为满足
Student entity limit 的拓扑/可保存性诊断，不是精度网格。

**不变量**：四入口、一出口、972 role queries、同一冻结 STEP、单 common fluid-zone、972 throat-axis
occupancy、free/multi-face、quality、cells/nodes 双上限、非空 `.msh.h5` 与 SHA 全部保留。若观察到
多 cell zones，必须 fail；禁止 merge/rename/fuse 伪装单区。

**分辨率边界**：0.25 mm throat diameter 只有约 3.3 个局部尺寸，0.10 mm throat length 约 1--2
个尺寸层，故即使 C1 PASS 也只证明拓扑与 Student 可执行性。精度和网格独立性必须由后续更高许可/
算力路线完成。

**真机结果**：volume-mesh API 完成，413,405 cells、min OQ 0.24604596；没有观察到
Student license error，但 node limit/完整 Student guard 因早期 fail closed 未执行。surface stage 为
`1 fluid/solid region and 11 voids`，现有 workflow 随后得到 12 fluid cell zones，各约 33,838--35,168
cells。consumer 在 `CELL_ZONE_COUNT_NOT_ONE` 处停止，未写 `.msh.h5`。这证明现有单区合同不满足，
但不证明 12 个都是应参与求解的流体区：11 个可能是 actuator dead/void pockets。下一步先
捕获 region state 并显式排除 dead/void；不能在 solver 中 merge/rename zones 伪造单区。

## AJM-P2-S0-001：中央锚固等效板 CAD 前置基线

日期：2026-07-15
状态：Windows SpaceClaim producer PASS；Mechanical/modal/harmonic `NOT_RUN`

**对象**：来自主布局 CELL_005 的 7×7 mm 单 cell 等效板，0.275 mm 候选板厚、2.25×2.25 mm
中央锚区域；它是结构校准子模型，不替代整机模型，也不声称为真实膜片叠层。

**实测结果**：job `AJM-P2-S0-EQ-M7-C005-e8f61480898c` 的 16/16 assertions 全真；Native 与
STEP 都保持 bbox、13.728125 mm3 union volume、single-piece/closed/manifold，Native 六组语义和 STEP
关键区域重建闭合。三个材料行 `EQ-A/B/C` 是 paired C-class sensitivity bundles，不是产品材料。

**下一 Gate**：在同一 MCP session 重新提交 producer 并冻结 predecessor，再运行 PyMechanical modal
smoke。首轮只允许写 pre-Gate candidate frequencies 和 normalized mode shape；P2、材料识别、谐响应和
物理位移幅值仍为 `NOT_RUN/NOT_CLAIMED`。
